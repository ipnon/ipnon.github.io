...
- **No `\useasboundingbox`, no JS — full visibility is structural.** Do not call `\useasboundingbox` in figures. Let TikZ compute the natural bounding box from drawn content; the SVG `viewBox` then equals that bbox by construction, so painted content cannot be outside the viewBox. CSS in `style.css` pins `.tikz-figure svg { width: 100% !important; height: auto !important; max-width: 100% }` — the `!important` is load-bearing because TikZJax sets `width` and `height` inline on the SVG, which would otherwise win over an external rule. Together, the SVG element sizes to the column and contains every painted glyph; clipping is structurally impossible. The tradeoff is that Figure 1 (with a hazard sticky) and Figure 2 (without) may render at slightly different aspect ratios within a post; this is acceptable for the no-clipping guarantee.
