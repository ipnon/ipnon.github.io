...
- **Clipping is handled at the layout level.** `_layouts/post.html` carries a small script that, after TikZJax renders, reads each SVG's real painted bounding box (`getBBox`) and grows the `viewBox` to the union of the original and that bbox. The SVG element then sizes to contain everything, `width: 100%` scales it down to the column, and content provably cannot spill past the column edge. Set `\useasboundingbox` to express layout intent (keeping Figure 1 and Figure 2 the same size for visual diff) — it is no longer load-bearing for visibility, and an overly tight rectangle is corrected automatically.
